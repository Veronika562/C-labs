...
void print_person(person_t* person, FILE* file) {
    ...
    fputs(person->city, file);
    fputs(person->education, file);
    fputs(person->job, file);
    fputs(person->interests, file);
    fputs("Contacts:\n", file);
    node_t* node = person->contacts.head;
    while (node != NULL) {
        char contact_name[100];
        strcpy(contact_name, " ");
        strcat(contact_name, node->person->name);
        fputs(contact_name, file);
        node = node->next;
    }
}
